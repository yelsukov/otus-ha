...
и выпадет из обоймы чтобы нового master выбрали, да и чтоб все бесшовно и без отрыва от процесса.
...
Самим кластером рулит Orchestrator, он же и производит выборы новой master ноды из доступных реплик, если старая master
нода приуныла.

Если интересно, то вот [детальный отчет по организации такой инфраструктуры](../replication/p2_semi_sync_replication.md)
.

Но так как данная задача про haproxy + до кучи необходима балансировка через nginx, то ниже по тексту реализация.
...
- **API Gateway** бывшая точка входа, аутентификации пользователей и авторизации запросов. На борту несет REST API. В
  комплекте два экземпляра, ими и будем масштабироваться.
...
Реплика будет настроена сервисом установщиком firestarter, который настроит semi-sync репликацию.

В процессе его работы будут исполнены следующие SQL запросы:
...
Увидим выдачу в консоль результаты выполнения команд `SHOW MASTER STATUS` и `SHOW SLAVE STATUS`.

Для мастера должны увидеть [что-то вроде этого](../replication/p2_semi_sync_replication.md#master-state)
...
Должны увидеть что оба сервиса взлетели и слушают порт `8080`
...
Тест будет запущен на роут поиска пользователей по префиксам имени и фамилии. Для рандомизации запроса будет
задействован вспомогательный [lua скрипт](../../wrk/request.lua).
...
Смотрим логи и видим, что запросы все еще идут и получают успешные ответы.
...
- Балансировка отработала успешно,- выжившие экземпляры сервиса и БД приняли на себя оставшуюся нагрузку и успешно
  отработали тест до конца.
...
5. Нагрузочное тестирование проведено. Результаты тестирования приведены. По результатам видно, что инфраструктуру
   показала определенную устойчивость к сбоям;
