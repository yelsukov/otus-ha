...
файл сервиса при процессе компиляции.
...
> **Обращаю внимание**, что версия API инкапсулирована в рамках роутера. Первая версия доступна по пути `/v1`.
> В случае выпуска новой мажорной версии API, его интерфейс будет доступен по пути `/v2`.
...
2021/03/15 12:45:53 [ddad7c64a23b/LGxTj7fF7r-000009] "GET http://dialogue:8080/messages?uid=3&cid=604f565622f02d3663a43d3b&limit=100 HTTP/1.1" from 192.168.16.6:38716 - 200 52B in 2.442122ms
2021/03/15 12:45:58 [ddad7c64a23b/LGxTj7fF7r-000010] "GET http://dialogue:8080/messages?uid=4&cid=604f565622f02d3663a43d3b&limit=100 HTTP/1.1" from 192.168.16.6:38716 - 200 153B in 6.170709ms
```

> **Обращаю внимание**, что в логах обоих сервисов присутствует сквозной идентификатор запроса `ddad7c64a23b/LGxTj7fF7r-[N]`**
> Идентификатор передается между сервисами через заголовок `X-Request-Id`.
> Так же любой ответ от API Gateway снабжен заголовком идентификатора запроса, что позволяет быстро определить весь ход запроса внутри системы.
