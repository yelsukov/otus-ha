...
Сидеру резко становится плохо и он прекращает запись, выдавая в консоль оишбку и конечный результат:
...
Видим, что `master_db` прилег ненадолго и хостгруппы сразу же перестроились на новый мастер хост `slave2_db`
...
<a name="my-pain"></a>
## Боль
Не хватило времен нормально разобраться с оркестратором, т.к. срок выполнения ДЗ затянулся до неприличия.
